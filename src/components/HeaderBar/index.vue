<template>
  ...
  <van-sticky>
    <header :class="[border_bottom ? 'border_bottom' : '']" :style="{ 'background-color': bgColor }">
      <div class="left">
        <!--左侧返回-->
        <van-icon name="arrow-left" color="var(--ex-default-font)" class="leftImg fw-bold" size="20" @click="toUrl" />
        <!--标题-->
        <div :class="isCenter ? 'centerStyle' : ''">{{ currentName }}</div>
        <!--插槽-->
        <slot name="titleName"></slot>
      </div>
      <div class="right" v-if="!['smartfund'].includes(_getConfig('_APP_ENV'))">
        <div class="rightBtn" v-if="JSON.stringify(cuttentRight) !== '{}'">
          <div v-if="cuttentRight.name" @click="linkTo">
            {{ cuttentRight.name }}
          </div>
          <image-load v-if="cuttentRight.icon" :filePath="cuttentRight.icon" class="rightBtnImg" @click="linkTo" />

          <div class="iconRightList" v-show="cuttentRight.iconRight" v-for="(item, index) in cuttentRight.iconRight"
            :key="index">
            <svg-load v-if="item.iconName" :name="item.iconName" class="rightIcon" @click="handelClick(item.clickTo)"
              :style="{ fontSize: item.iconSize }"></svg-load>
