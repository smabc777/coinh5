...
export const useTradeStore = defineStore('trade', {
  state: () => {
    return {
      ...
      preciousMetalsList: reactive([]),
      foreignExchangeList: reactive([]),
  ...
  actions: {
    ...
    async getCoinList() {
      ...
      let keyMap = {
        ...
        foreignExchangeList:'foreignExchangeList',
        preciousMetalsList:'preciousMetalsList'
        // stockList: 'stockList'
      }
      ...
      this.preciousMetalsList = []
      this.foreignExchangeList = []
      this.stockList = []
      for (const key in keyMap) {
        if (res?.data && Object.hasOwnProperty.call(res.data, key)) {
          ...
          tempList.forEach((elem) => {
            ...
            /* let filterCoin = this[keyMap[key]].filter((itms, idx) => {
              return itms.coin === elem.coin
            })
            ...
            // 国际黄金白银
            if (elem.coinType === 3 || elem.coinType === 5) {
        ...
        this.token = PubSub.subscribe(socketDict.DETAIL, (key, data) => {
          ...
          if (['coinsexpto'].includes(__config._APP_ENV)) {
            if (this.currentCoinList.includes(data.symbol) && data.origin != 'kline') {
              // console.log(this.currentCoinList)
