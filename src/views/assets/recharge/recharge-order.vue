<template>
  <div class="recharge-order">
    <!-- 充值记录 -->
    <HeaderBar :currentName="_t18('recharge_order',['vitc'])"></HeaderBar>
    <Tab :tabList="tabList" :active="curIndex" @change="changeIndex" :lineWidth="30">
      <template #tabContent>
        <!-- 下拉刷新 -->
        <van-pull-refresh
          v-model="refreshing"
        ...
        >
          ...
          <div v-else>
            ...
            <van-list
              ...
              @load="onLoad"
            >
              <div class="record-list" v-for="(item, index) in tabContentList" :key="index">
...
import OrderList from '../components/record/OrderList.vue'
...
const tabList = computed(() => {
  ...
  // list = ['全部', '审核', '成功', '失败']
  list = [
    _t18('recharge_tab_all', ['aams','vitc']),
    _t18('recharge_tab_wait',['vitc']),
