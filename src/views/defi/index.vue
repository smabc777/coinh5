    ...
    <div class="profit">
      <div class="tit">
        ...
        <div class="rule" @click="showPopup">
          <van-icon name="info-o" color="var(--ex-eth-button-color)" class="icon"/>
          {{
            _t18('defi_speak')
          }}
        </div>
...
const getIncome = async () => {
  if (userStore.userInfo.approve == 1) {
    let res = await getIncomeApi(userStore.userInfo.user.userId,'usdt')
    if (res.code == 200) {
