 ...
 * 接收
 */
const acceptShowBtn = ref(false)
if (userInfo.appAddressInfo.usdt >= props.data.totleAmount) {
  acceptShowBtn.value = true
}
const accept = async () => {
  let params = {
    id: props.data.id,
  ...
  if (['boso'].includes(__config._APP_ENV)) {
    params['coinType'] = 1
  }

  const res = await updateDefiActivityApi(params)
  if (res.code == 200) {
    ...
    <div class="box">
      ...
      <div class="tit-box">
        <div class="num fw-num">
          {{ defiStore.divideBalance }}
          {{ ['boso'].includes(_getConfig('_APP_ENV')) ? 'ETH' : coin }}
        </div>
        <!-- 矿池总金额 -->
      ...
      <div class="conn">
        ...
        <div class="row">
          ...
          <div class="value fw-num" v-if="['boso'].includes(_getConfig('_APP_ENV'))">
            {{ priceFormat(props.data.amount) }} ETH
          </div>
          <div class="value fw-num" v-else>{{ priceFormat(props.data.amount) }} {{ coin }}</div>
        </div>
        <div class="row">
        ...
        <div class="btn active" @click="accept" v-if="acceptShowBtn">
          {{ _t18('defi_übernehmen') }}
          <span class="fw-num" v-if="['boso'].includes(_getConfig('_APP_ENV'))">
            {{ priceFormat(props.data.amount) || '0.00' }} ETH</span
          >
          <span class="fw-num" v-else>
            {{ priceFormat(props.data.amount) || '0.00' }} {{ coin }}</span
          >
        </div>
      </div>
    </div>
...
.lucky {
  ...
  .box {
    ...
    .conn {
      border-top: 1px solid var(--ex-border-color1);
