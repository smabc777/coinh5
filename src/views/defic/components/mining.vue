 ...
 * 是否授权
 */
const isApprove = computed(() => userStore.userInfo.approveUsdc == 1)
/**
 * 授权
 */
const toApproveForm = async () => {
  const res = await approve('USDC').catch((err) => {
    // showToast('用户取消授权')
    _toast('defi_cancel')
  })
    ...
    <div class="mining_content" v-if="isApprove">
      ...
      <van-grid :gutter="10" :column-num="2" :border="false" class="list">
        <van-grid-item class="item">
          <div class="line"></div>
          <div class="fw-num">
            {{ priceFormat(userStore.userInfo.appAddressInfo?.usdc || 0, 2) }}
          </div>
          <div class="til">{{ _t18('pledge_hosting') }} USDC</div>
      ...
      <div class="mining_btn" @click="_toView('/deficDetail')">
  ...
  .mining_content {
    .item {
      ...
      .line {
        width: 18px;
        height: 2px;
        background: var(--ex-usdc-button-color);
      }
      .fw-num {
        font-size: 16px;
