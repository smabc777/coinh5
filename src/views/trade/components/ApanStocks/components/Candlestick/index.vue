  ...
  <div>
    <div class="third">
      <div class="list">
        ...
        <!-- <div
          class="thirdRight"
          v-show="currentStockInfo.market != 'mt5'"
...
<script setup>
import { getKlineStockHistory } from '@/api/common/kline.js'
import Datafees from '@/config/kline/datafees.js'
import { getConfig, getIntervalList, getStudyList } from '@/config/kline/index1.js'
import { _coinWebSocket } from '@/plugin/socket/index'
import { _t18, _klineTimeFormat } from '@/utils/public'
import { socketDict } from '@/config/dict'
import PubSub from 'pubsub-js'
import { _add, _div, _mul, _sub, priceFormat, _toFixed } from '@/utils/decimal'
...
const priceScale = computed(() => {
  let tempPricescale = 100
  // console.log(stockInfo.value?.amount, 'nnnnnnnnn787877');

  // if (stockInfo.value?.amount > 100) {
  //   tempPricescale = 100
  // } else if (stockInfo.value?.amount < 10) {
  //   tempPricescale = 1000000
  // } else if (stockInfo.value?.amount < 100) {
...
const eventTradeSymbolChange = debounce((e) => {
  ...
  setSymbol(symbol, currentInterval.interval, () => {
    stockStore.setCurrentStockList(currentStockInfo.coin, 'remove')
    Object.assign(currentStockInfo, stockInfo.value)
    stockStore.setCurrentStockList(currentStockInfo.coin)
  })
 ...
 * 获取分辨率
 */
const getSupportedResolutions = (tempCoinInfo = currentStockInfo) => {
  let tempList = intervalList.map((elem) => elem.interval)
  // console.log(tempList, 'zzzzzzzzzzzzzzzzzzzz')
...
const getHeadIntervalList = (tempCoinInfo = currentStockInfo) => {
...
watch(
  ...
  () => {
    ...
    if(route.query.tradeKey == 'ApanStocks'){
//初始化
initialize()
    }
...
watch(
  ...
  (no, ol) => {
    if (no.indexOf("/trade?tradeKey=ApanStocks") != -1 && !flag.value) {
      flag.value = true
      initialize()
    }
  }
...
const setSymbol = async (symbol, interval, callBack) => {
  ...
  // console.log('----设置时----', (currentStockInfo.symbol != symbol || interval != currentInterval.interval))

  if (currentStockInfo.symbol != symbol || interval != currentInterval.interval) {

    widget?.setSymbol(symbol, interval, async () => {
      // console.log('------setSymbol---------', stockInfo.value.symbol, symbol, interval)
 ...
 * 获取历史KLine数据
 */
const flag = ref(true) // 控制递归定时器是否继续的标志

dataFeedInstance.getBars = async ({ symbolInfo: stockInfo, resolution, from, firstDataRequest }) => {

  let tempInterval = intervalList.find((elem) => elem.interval == resolution)
  try {
    if (tempInterval && (from == '' || from > 0)) {
      let params = {
        symbol: stockInfo.symbolUpperCase,
      ...
      console.log(params, stockInfo, 'dddddddddddddddddddddddddd')
      // let barList = await client.candles(params)
      const { data } = await getKlineStockHistory({
        ...params,
        page: 1,
        interval: tempInterval.key,
        symbol: stockInfo.symbolUpperCase,
      ...
      /* stockStore.setKlineTicker(data.ticker)
      ...
      // dateToMs(dayjs().tz(mainStore.timezone).format(mainStore.timeFormat + ' HH:mm:ss'))
      barList = barList
        .map((elem) => {
          return {
...
dataFeedInstance.resolveSymbol = async () => {
  return {
    // 商品名称
    name: stockInfo.value.symbol,
    // 自定义携带参数
    coin: stockInfo.value.coin,
    coinUpperCase: stockInfo.value.coin,
    symbol: stockInfo.value.symbol,
    symbolUpperCase: stockInfo.value.symbol,
    market: stockInfo.value.market,
    stockType: stockInfo.value.stockType,
    // resolution: '1m',
    fractional: false,
    //设置周期
    session: '24x7',
    ...
    //设置是否支持周月线
    description: stockInfo.value.symbol,
...
const initWidget = () => {
  ...
  widget = new TradingView.widget({
    symbol: stockInfo.value.symbolUpperCase,
    theme,
    debug: false,
    autosize: true,
    // 默认分辨率
...
const unsubscribeTrades = (firstDataRequest = false) => {
  if (currentStockInfo.coin) {
    _coinWebSocket.send({
      op: socketDict.unsubscribe,
      type: socketDict.KLINE,
      symbol: currentStockInfo.coin,
...
const subscribeTrades = async (params) => {
  ...
  let candlestickKey = PubSub.subscribe(socketDict.KLINE, (key, data) => {
    ...
    if (data.symbol == params.coin?.toLocaleLowerCase()) {
      ...
      if (currentStockInfo.market == 'metal') {
        // 期货数据
      ...
      if (currentStockInfo.market == 'mt5') {
        tempTrade.high = tempData.close
        tempTrade.low = tempData.close
      } else {
...
const checkedInterval = (item, type = '') => {
  ...
  if (item.interval != currentInterval.interval) {
    setSymbol(stockInfo.value.symbol, item.interval)
  }
  showMenu.value = !!type
}
