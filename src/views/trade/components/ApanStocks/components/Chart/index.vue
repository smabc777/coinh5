  ...
  <div class="handicapBox">
    ...
    <div class="title">
      <div class="title_left">
        ...
        <div class="unit">({{ stockInfo.baseCoin }})</div>
      </div>
    ...
    <div class="leftList">
      <div class="itemList asks" v-show="showType.value != 2">
        <!-- 卖出盘 -->
        <div class="item" v-for="(item, index) in rows" @click="emits('setTradePrice', asksList[index]?.price)"
          :key="`asks${index}`">
          <div class="fall fw-num">{{ priceFormat(asksList[index]?.price) }}</div>
          <div class="itemName fw-num">{{ countFormat(asksList[index]?.count) }}</div>
          <div class="hightItem fall rfd-bg" :style="`width:${_toFixed(
      ...
      <div class="itemCenter">
        <div :class="[
          _isRFD(stockDetailInfo?.open, stockDetailInfo?.amount, 'buy', 'rise'),
          'fw-num centername'
        ]" @click="emits('setTradePrice', stockDetailInfo?.amount)">
          {{ priceFormat(stockDetailInfo?.amount) }}
        </div>
        <div class="">≈${{ stockDetailInfo?.amount }}</div>
      </div>
      <div class="itemList" v-show="showType.value != 1">
        <!-- 买入盘 -->
        <div class="item itemHight" v-for="(item, index) in rows" :key="`bids${index}`"
          ...
          <div class="hightItem rise rfd-bg" :style="`width:${_toFixed(
    ...
    <div class="leftBottom">
      ...
      <div class="bottomLeft" @click="showDepthPopup = true">
        <div class="fw-num">{{ showDepth?.name }}</div>
        <van-icon name="play" size="14" color="var(--ex-font-color18)" class="img" />
      </div>
      <!-- 展示盘口 -->
    ...
    <!-- 选择聚合深度 -->
    <van-action-sheet v-model:show="showDepthPopup" teleport="#app" :actions="showDepthList"
      @select="_checkedShowDepth" />
    <!-- 展示盘 -->
    <van-action-sheet v-model:show="showTypePopup" teleport="#app" :actions="showTypeList" @select="_checkedShowType" />
  </div>
</template>
...
const showTypeList = ref([
  // 默认
  {
    name: _showName(`bb_default`),
    value: 0,
    color: 'var(--ex-default-font-color)',
    icon: 'morenpan'
  },
  // 卖出盘
  { name: _showName(`bb_sell_order`), value: 1, color: '', icon: 'maichupan' },
  // 买入盘
  { name: _showName(`bb_buy_order`), value: 2, color: '', icon: 'mairupan' }
])
/**
 * 当前显示类型
 */
const showType = ref(showTypeList.value[0])
/**
 * 显示Popup变量
 */
...
const _checkedShowType = (item) => {
  //清空颜色
  showTypeList.value.forEach((items) => {
    if (items.name == item.name) {
      items.color = 'var(--ex-default-font-color)'
    } else {
      items.color = ''
    }
  })
  if (item.name != showType.value.name) {
    showType.value = item
  }
 ...
 * 盘口显示类型
 */
const showDepthListDict = ref([
  { name: '0.00000001', value: 0, color: '', min: 0.00000001 },
  { name: '0.0000001', value: 1, color: '', min: 0.0000001 },
  { name: '0.000001', value: 2, color: '', min: 0.000001 },
  { name: '0.00001', value: 3, color: '', min: 0.00001 },
  { name: '0.0001', value: 4, color: '', min: 0.0001 },
  { name: '0.001', value: 5, color: '', min: 0.001 },
  { name: '0.01', value: 6, color: '', min: 0.01 },
  { name: '0.1', value: 7, color: '', min: 0.1 },
  { name: '1', value: 8, color: '', min: 10 },
  { name: '10', value: 9, color: '', min: 100 },
  { name: '100', value: 10, color: '', min: 1000 }
])
...
const _checkedShowDepth = (item) => {
  //清空颜色
  showDepthList.value.forEach((items) => {
    if (items.name == item.name) {
      items.color = 'var(--ex-default-font-color)'
    } else {
      items.color = ''
...
const getMockDataList = () => {
  ...
  let price = stockDetailInfo.value?.amount
 ...
 * 交易详情数据
 */
const stockDetailInfo = computed(() => stockStore.stockInfo)

watch(
  () => stockInfo.value?.coin,
  () => {
    // console.log('股票更换', stockInfo.value, 'dfffffffffffffffffffff')
    // console.log('币种更换', stockInfo.value?.coin, props.rows, stockDetailInfo.value.close)
    // 关盘价大于0
    if (stockDetailInfo.value?.amount > 0) {
      showDepthList.value = showDepthListDict.value
        .filter((elem) => elem.min * 30 < stockDetailInfo.value.amount)
        .slice(-4)
      showDepthList.value[0].color = 'var(--ex-default-font-color)'
...
const subKey = PubSub.subscribe(socketDict.DETAIL, (key, data) => {
  if (data.symbol && data.symbol == stockInfo.value.coin) {
    if (stockDetailInfo.value.amount && showDepthList.value.length) {
      getMockDataListThrottle()
    } else {
      showDepthList.value = showDepthListDict.value
        .filter((elem) => elem.min * 30 < stockDetailInfo.value.amount)
...
.leftList {
  ...
  .itemList {
    ...
    .item {
      ...
      .itemName {
        color: var(--ex-trade-chart-text-color);
      }
