<template>
  <div>
    <div class="top">
      <div class="first">
        <!-- 打开弹窗 -->
        <div class="firLeft" @click="QuoteSidebarRef.setQuoteSidebarData()">
          <svg-load name="cebian17x14" class="firLeftImg"></svg-load>
          <div class="fw-bold">{{ stockStore.stockInfo.symbol }}</div>
        </div>
        <!-- U本位规则-->
        <div class="first">
          <!-- <svg-load name="shuaxin" class="senLeftImg" @click="kLineRefresh"></svg-load> -->

          <!-- <svg-load name="k-xian" class="senLeftImg"
            @click="$router.push(`/stockDetail?symbol=${stockInfo.coin}&type=9`)"></svg-load> -->
          <svg-load v-if="!['cmmetrics'].includes(_getConfig('_APP_ENV'))" name="guize" class="senLeftImg"
            @click="_toView('/tradingRules?type=4')"></svg-load>
        </div>
      </div>
    </div>
