...
import {countOrderTotals, getOrderListCurrencyApi} from '@/api/trade'
...
const tabList = computed(()=>[
  {label: _showName(`exchange_curentrust`), value: 0, num: getOrderTotleList.value?.openOrder || 0},
  {label: _showName(`exchange_hisentrust`), value: 1, num:  getOrderTotleList.value?.orderHistory || 0}
])
...
const getOrderListNumber = async () => {
  ...
  //当前委托/历史委托/订单数量  更多......
  const res = await countOrderTotals('B_TOTAL')
