<template>
  <div class="handicapBox">
    ...
    <!-- 选择聚合深度 -->
    <van-action-sheet
        v-model:show="showDepthPopup"
        teleport="#app"
        :actions="showDepthList"
        @select="_checkedShowDepth"/>
    <!-- 展示盘 -->
    <van-action-sheet
        v-model:show="showTypePopup"
        teleport="#app"
        :actions="showTypeList"
        @select="_checkedShowType"/>
  </div>
</template>
...
const showTypeList = ref([
  // 默认
  {name: _showName(`bb_default`), value: 0, color: 'var(--ex-default-font-color)', icon: 'morenpan'},
  // 卖出盘
...
watch(
    () => coinInfo.value?.coin,
    () => {
      console.log(tradeDetailInfo.value)
      // console.log('币种更换', coinInfo.value?.coin, props.rows, tradeDetailInfo.value.close)
...
const subKey = PubSub.subscribe(socketDict.DETAIL, (key, data) => {
  if (data.symbol && data.symbol == coinInfo.value.coin) {
