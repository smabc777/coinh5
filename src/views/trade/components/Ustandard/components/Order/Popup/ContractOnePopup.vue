<template>
  <!-- 追加合约保证金 -->
  <PublicPopup :show="showContractOne"
               teleport="#app"
               position="absolute"
               :direction="`bottom`"
               :width="'var(--ex-max-width)'"
               @handelClose="showContractOneClose">
...
import {adjustPositionMargn} from "@/api/trade";
...
//初始化
const ContractOnePopupInit = (item, availableBalances) => {
  PositionMargn.value = ''
  PositionAmout.value = ''
...
const adjustMargn = () => {
  ...
  adjustPositionMargn(data, {loading:true}).then((res) => {
    if (res.code == 200) {
      showContractOne.value = false
      showToast(res.msg)
      refreshOrder()
